...
switch ($action) {
    ...
    case 'sync_payments':
        // 1. First, ensure this is a POST request, as it involves writing data.
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        ...
        // 2. Read the raw JSON data sent from the app in the request body.
        $payments_from_app = json_decode(file_get_contents('php://input'), true);

        // 3. Prepare an empty array to send back to the app.
        $sync_results = [];

        // 4. Check if the JSON was successfully decoded and is an array.
        if (is_array($payments_from_app)) {
            // 5. Prepare the SQL INSERT statement once for efficiency.
            //    Using '?' placeholders is crucial for preventing SQL injection attacks.
            $stmt = $mysqli->prepare(
                "INSERT INTO payments (local_id, account_id, user_id, amount, timestamp, device_id) VALUES (?, ?, ?, ?, ?, ?)"
            );

            if ($stmt) {
                // 6. Loop through each payment object that the app sent.
                foreach ($payments_from_app as $payment) {
                    // The app sends a Unix timestamp in milliseconds. We divide by 1000
                    // to get seconds, which PHP's date() function requires.
                    $datetime = date("Y-m-d H:i:s", $payment['timestamp'] / 1000);

                    // 7. Bind the values from the payment object to the '?' placeholders.
                    //    The "iiiiss" string tells the database the data types are:
                    ...
                    // 8. Execute the prepared statement to insert the data.
                    if ($stmt->execute()) {
                        // 9. If the insert was successful, create a result object.
                        //    This maps the app's original 'localId' to the new 'serverId'
                        //    that was just generated by the database. The app needs this.
                        $sync_results[] = [
                ...
                // 10. Close the statement when done.
                $stmt->close();
            }
        }

        // 11. Finally, encode the results array into JSON and send it back to the app.
        //     If everything worked, this will be a list of {localId, serverId} pairs.
        //     If nothing was synced, this will be an empty array: []
